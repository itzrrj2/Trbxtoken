...
async def start_command(client, message):
    ...
    if verify_status["is_verified"]:
        ...
        if IS_VERIFY:
            ...
            tutorial_button = InlineKeyboardButton("How to Verify 🎥", url="https://t.me/AR_File_To_Link_Bot/?start=MzA2OTMxOTgxMzI2MzkwOTAwLzczMTI4MjAxNg")
...
async def handle_message(client, message: Message):
    ...
    if message.from_user is None:
        logging.warning("Received a message with no 'from_user'. Ignoring.")
        return
...
async def safe_edit_message(message, text, reply_markup=None):
    """Safely edit a message, handling invalid or deleted messages."""
    try:
        await message.edit_text(text, reply_markup=reply_markup)
    except pyrogram.errors.MessageIdInvalid:
        logging.warning("Attempted to edit an invalid or deleted message.")
    except Exception as e:
